...
assistant = client.beta.assistants.create(
  name="F0X Mods Assistant",
  instructions="You are a helpful assistant. You are in the Discord server of the mod developer WereF0X, also known as Fox, known on CurseForge and Modrinth. You can help assisting members and providing code assistance.",
...
ticket_channels = {}  # Dictionary to keep track of ticket channels
...
async def claim_ticket(ctx):
    ...
    ticket_creator = discord.utils.get(ctx.guild.members, id=list(ticket_channels.keys())[list(ticket_channels.values()).index(ctx.channel.id)])
    if ticket_creator:
...
async def close_ticket(ctx):
    ...
    del ticket_channels[list(ticket_channels.keys())[list(ticket_channels.values()).index(ctx.channel.id)]]
