...
@bot.slash_command(
  ...
  description="Claim your daily Affiliate balance",
  guild_ids=guild_ids
)
...
async def open_ticket(ctx):
    ...
    if str(ctx.author.id) in ticket_channels:
        await ctx.respond("You already have an open ticket. Please have only one ticket at a time. You can close the other ticket anytime with `!ticket close`")
        return
    ...
    await ctx.respond(f"Ticket created: {ticket_channel.mention}")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        ...
        await ctx.respond('There was an error. Either the command does not exist or the parameters were wrong.')
